...
(function () {
  d3.timeline = function() {
	...
	var wrapRectLabel = function() {
		
		var self = d3.select(this);
		var parentClass = self[0][0].getAttribute("data-parent-class");
		
		if(parentClass) {
			
			var parentSelector = parentClass.split(" ").join(".");
			var parentElem = d3.select("." + parentSelector)[0][0];
    ...
    var appendLabel = function (gParent, yAxisMapping, index, hasLabel, datum) {
      ...
      gParent.append("text")
		...
		.append('tspan').text(hasLabel ? labelFunction(datum.label) : datum.id).each(wrapLabel);
    };
    ...
    function timeline (gParent) {
        ...
        d.forEach( function(datum, index){
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .attr("class", function (d, i) {
				d.customClass = datum.class ? "timelineSeries_" +  datum.class : "timelineSeries_"+ index;
				if(d.class) d.customClass +=  (" " + d.class);
				if(d.id) d.customClass +=  (" text" + d.id);
          ...
          g.selectAll("svg").data(data).enter()
            .append("text")
			.attr("data-parent-class", getRectParentClass)
	  ...
	  function getRectParentClass(d, i) {
		return d.customClass;
	  }
