...
/**
 * Performance test for the EmployeeAbsentReason entity.
 */
class EmployeeAbsentReasonGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the EmployeeAbsentReason entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all employeeAbsentReasons")
            .get("/api/employee-absent-reasons")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            ...
            .pause(10)
            .repeat(5) {
                exec(http("Get created employeeAbsentReason")
                .get("${new_employeeAbsentReason_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created employeeAbsentReason")
            .delete("${new_employeeAbsentReason_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
